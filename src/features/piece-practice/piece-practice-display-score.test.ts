...
const piece: PiecePracticePiece = {
  ...
  measures: [{ measureIndex: 0, sourceMeasureId: "m1", absoluteStartTick: 0, capacityTicks: 1920, keySignatureId: "c-major", timeSignature: "4/4", restEventIds: [], targets: [], sourceEvents: [{
    sourceEventId: "from", kind: "notes", staff: "treble", startTick: 1440, absoluteStartTick: 1440, duration: "quarter", durationTicks: 480,
    pitches: [{ sourcePitchId: "from-p", midiNumber: 60, letter: "C", accidental: "natural", octave: 4, incomingTieIds: [], outgoingTieIds: ["tie"], requiresAttack: true }],
...
describe("Piece Practice display score", () => {
  it("reconstructs read-only notation context, source IDs, and ties", () => {
    expect(createPiecePracticeDisplayScore(piece)).toMatchObject({
      ...
      measures: [{ id: "m1", events: [{ id: "from" }] }, { id: "m2", keySignatureChange: "g-major", timeSignatureChange: "3/4", events: [{ id: "to" }] }],
