...
export const MelodyResults = forwardRef<HTMLHeadingElement, Readonly<{ exercise: MelodyExercise; result: MelodyAttemptResult; onRetrySame: () => void; onTryAnother: () => void; onSettings: () => void }>>(function MelodyResults({ exercise, result, onRetrySame, onTryAnother, onSettings }, ref) {
  ...
  </div><section aria-labelledby="melody-pitch-results-title" className="space-y-3"><div><h3 id="melody-pitch-results-title">Pitch results on the staff</h3><p>The staff below shows pitch results. Timing is scored separately.</p></div><div aria-label="Pitch result legend" className="melody-result-legend"><span><i aria-hidden="true" className="melody-result-swatch melody-result-swatch-correct" />Correct</span><span><i aria-hidden="true" className="melody-result-swatch melody-result-swatch-missed" />Missed</span><span><i aria-hidden="true" className="melody-result-swatch melody-result-swatch-wrong-pitch" />Wrong pitch</span></div><div aria-label="Melody pitch result score" className="melody-score-scroll" data-measure-count={exercise.measures.length} tabIndex={0}><div className="melody-score-track"><div className="melody-score-measures grid gap-3" style={{ gridTemplateColumns: `repeat(${exercise.measures.length}, minmax(0, 1fr))` }}>{exercise.measures.map((measure) => <StaffBuilderScoreView eventHighlights={highlights} key={measure.id} measureIndex={measure.measureIndex} score={score} visibleStaff={exercise.settings.staff} />)}</div></div></div><ul aria-label="Pitch result details" className="melody-result-details">{details.map(({ eventId, text }) => <li key={eventId}>{text}</li>)}</ul></section><p>Missed: {result.missedAttackCount} · Extra: {result.extraAttackCount}</p><div className="melody-result-actions flex flex-wrap gap-2"><button onClick={onRetrySame}>Retry Same</button><button onClick={onTryAnother}>Try Another</button><button onClick={onSettings}>Settings</button></div></section>;
});
