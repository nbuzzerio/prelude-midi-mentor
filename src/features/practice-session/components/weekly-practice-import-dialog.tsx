...
export default function WeeklyPracticeImportDialog({ library, createId, onCancel, onImport }: Props) {
  ...
  const dialogRef = useRef<HTMLDivElement>(null); const titleRef = useRef<HTMLHeadingElement>(null);
  ...
  const keyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    const controls = [...(dialogRef.current?.querySelectorAll<HTMLElement>('button, textarea, input:not([disabled]), [tabindex]:not([tabindex="-1"])') ?? [])];
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3 sm:p-6">
    <div aria-describedby="weekly-import-description" aria-labelledby="weekly-import-title" aria-modal="true" className="flex max-h-[92vh] w-full max-w-4xl flex-col rounded-xl border border-white/15 bg-zinc-950 p-4 shadow-2xl sm:p-6" onKeyDown={keyDown} ref={dialogRef} role="dialog">
      <header><h2 className="text-xl font-bold" id="weekly-import-title" ref={titleRef} tabIndex={-1}>Import Weekly Plan</h2><p className="mt-1 text-sm text-zinc-400" id="weekly-import-description">Paste raw JSON or choose a .json file. Nothing is added until you confirm.</p></header>
      ...
      <footer className="mt-4 flex flex-wrap justify-end gap-3 border-t border-white/10 pt-4">{candidate && <button className="min-h-11 rounded bg-sky-500 px-4 font-semibold disabled:opacity-40" disabled={nameIssues.length > 0} onClick={confirm} type="button">Import weekly plan</button>}<button className="min-h-11 rounded border border-zinc-600 px-4" onClick={onCancel} type="button">Cancel</button></footer>
    </div>
  </div>;
}
