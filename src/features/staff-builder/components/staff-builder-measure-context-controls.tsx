...
export function StaffBuilderMeasureContextControls({ score, measureIndex, onKeyChange, onTimeChange }: Readonly<{
  ...
  return <fieldset className="staff-builder-context-controls"><legend>Measure {measureIndex + 1} context</legend>
    <label>Key signature <select className="staff-builder-input" onChange={(event) => onKeyChange(measureIndex, event.target.value === "inherit" ? null : event.target.value as MusicKeyId)} value={keyValue}>{measureIndex > 0 && <option value="inherit">Inherit ({context.keySignatureId})</option>}{MUSIC_KEYS.map((key) => <option key={key.id} value={key.id}>Use {key.name}</option>)}</select></label>
