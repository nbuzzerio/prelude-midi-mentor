...
describe("StaffBuilderCaptureStrip", () => {
  const actions = () => ({ step: vi.fn(), previous: vi.fn(), lock: vi.fn(), next: vi.fn(), clear: vi.fn() });

  it("uses glyph steps and decorative Lucide icons while preserving callbacks", () => {
    const a = actions();
    render(<StaffBuilderCaptureStrip captureState={DEFAULT_STAFF_BUILDER_CAPTURE_STATE} hasPending onClear={a.clear} onLock={a.lock} onNext={a.next} onPrevious={a.previous} onStepDurationChange={a.step} />);
  ...
  it("disables Previous at origin and only shows Clear with pending pitches", () => {
    const a = actions();
    const { rerender } = render(<StaffBuilderCaptureStrip captureState={DEFAULT_STAFF_BUILDER_CAPTURE_STATE} hasPending={false} onClear={a.clear} onLock={a.lock} onNext={a.next} onPrevious={a.previous} onStepDurationChange={a.step} />);
    ...
    rerender(<StaffBuilderCaptureStrip captureState={{ ...DEFAULT_STAFF_BUILDER_CAPTURE_STATE, cursor: { measureIndex: 0, offsetTicks: 240 } }} hasPending onClear={a.clear} onLock={a.lock} onNext={a.next} onPrevious={a.previous} onStepDurationChange={a.step} />);
