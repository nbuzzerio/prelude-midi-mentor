...
function getStaffBuilderDurationWheelPlacement(anchor: StaffBuilderEventAnchor, space: Readonly<{ width: number; height: number }>) {
  const preferredLeft = anchor.x + anchor.width + 10;
  const left = preferredLeft + WIDTH <= space.width ? preferredLeft : anchor.x - WIDTH - 10;
  return { left: Math.max(0, Math.min(left, space.width - WIDTH)), top: Math.max(0, Math.min(anchor.y - 18, space.height - HEIGHT)) };
}

export function StaffBuilderDurationWheel({ anchor, coordinateSpace, eventKind, currentDuration, onChoose, onClose }: Readonly<{
  ...
  const placement = getStaffBuilderDurationWheelPlacement(anchor, coordinateSpace);
