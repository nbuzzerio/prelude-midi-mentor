...
describe("StaffBuilderLibrary", () => {
  it("places Print / PDF after Download and before Duplicate and returns the exact saved score", () => {
    ...
    const { container } = render(<StaffBuilderLibrary activePieceId={null} onDelete={vi.fn()} onDownload={vi.fn()} onDuplicate={vi.fn()} onImportFile={vi.fn()} onOpen={vi.fn()} onPractice={vi.fn()} onPrint={print} onRename={vi.fn()} pieces={[piece]} />);
  ...
  it("keeps the existing primary and secondary actions available", () => {
    render(<StaffBuilderLibrary activePieceId={null} onDelete={vi.fn()} onDownload={vi.fn()} onDuplicate={vi.fn()} onImportFile={vi.fn()} onOpen={vi.fn()} onPractice={vi.fn()} onPrint={vi.fn()} onRename={vi.fn()} pieces={[piece]} />);
