...
export function StaffBuilderWorkspacePlaceholder({ score, initialCaptureState, initialEditorPass, initialRhythmState, onDraftChange, onValidatedSave, onClose, savingAvailable }: Readonly<{
  ...
  return <section className="staff-builder-panel" aria-labelledby="staff-builder-workspace-title">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-semibold" id="staff-builder-workspace-title">{editor.score.title}</h2><p className={savingAvailable ? "text-sky-300" : "text-amber-300"}>{savingAvailable ? `${editor.validation.issues.length === 0 ? "Ready to validate" : "Needs validation"} · Draft autosave active` : "In memory · Local saving unavailable"}</p></div><button className="staff-builder-secondary-button" onClick={() => { if (window.confirm("Close this piece and clear its active draft?")) onClose(); }} type="button">Back to Library</button></div>
    <dl className="staff-builder-metadata"><div><dt>Initial key</dt><dd>{getMusicKeyDefinition(editor.score.initialKeySignatureId).name}</dd></div><div><dt>Initial time</dt><dd>{editor.score.initialTimeSignature}</dd></div><div><dt>Tempo</dt><dd>{editor.score.tempoBpm} BPM</dd></div></dl>
    <div className="staff-builder-pass-switcher" aria-label="Editor pass"><button aria-pressed={!editor.validation.active && editor.editorPass === "capture"} className="staff-builder-secondary-button" onClick={editor.switchToCapture} type="button">Fast Capture</button><button aria-pressed={!editor.validation.active && editor.editorPass === "rhythm"} className="staff-builder-secondary-button" disabled={!editor.canEnterRhythm} onClick={editor.switchToRhythm} type="button">Rhythm Correction</button></div>
    ...
    <StaffBuilderMeasureContextControls measureIndex={visibleMeasureIndex} onKeyChange={editor.setMeasureKey} onTimeChange={editor.setMeasureTime} score={editor.score} />
    ...
    {editor.validation.active
      ? <StaffBuilderValidationPanel activeIndex={editor.validation.activeIssueIndex} activeIssue={editor.validation.activeIssue} issues={editor.validation.issues} onActivate={editor.validation.activate} onClose={editor.validation.close} onCorrection={editor.validation.applyCorrection} onNext={editor.validation.next} onPrevious={editor.validation.previous} status={editor.validation.status} />
