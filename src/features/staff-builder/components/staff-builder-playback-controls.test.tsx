...
describe("StaffBuilderPlaybackControls", () => {
  it("invokes every available playback scope and exposes shared volume", () => {
    const callbacks = renderControls();
    fireEvent.click(screen.getByRole("button", { name: "Audition Selected Event" }));
    fireEvent.click(screen.getByRole("button", { name: "Play Current Measure" }));
    fireEvent.click(screen.getByRole("button", { name: "Play From Here" }));
    fireEvent.click(screen.getByRole("button", { name: "Play Entire Piece" }));
  ...
  it("explains unavailable rest, unresolved, and non-Rhythm auditions", () => {
    ...
    expect(screen.getByText("Rests do not have pitches to audition.")).toBeTruthy();
    ...
    expect(screen.getByText(/Choose Rhythm Correction/)).toBeTruthy();
  });
  ...
  it("keeps Stop reachable while playing and announces failure", () => {
    ...
    expect(screen.getByRole("button", { name: "Play Entire Piece" }).getAttribute("aria-pressed")).toBe("true");
