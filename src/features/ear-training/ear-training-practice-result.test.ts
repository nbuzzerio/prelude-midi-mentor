...
import { appendEarTrainingCompletedTarget, appendEarTrainingIncorrectGuess, createEarTrainingPracticeResult, selectEarTrainingDiagnosticChips, selectEarTrainingPracticeReport, selectEarTrainingPracticeReportPhases, snapshotEarTrainingTarget } from "./ear-training-practice-result";
...
describe("Ear Training practice result", () => {
  ...
  it("selects identification, first-guess, retry, and response-time reporting", () => {
    ...
    const chips = selectEarTrainingDiagnosticChips(report);
    expect(chips.map(({ id, count }) => [id, count])).toEqual([["identification", 1], ["retried", 1]]);
    expect(chips.some(({ id }) => id === "pitch-problem" || id === "hesitation")).toBe(false);
  });
