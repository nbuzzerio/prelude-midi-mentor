...
describe("App focus mode", () => {
  ...
  it("exposes the package version in the persistent mode navigation", () => {
    render(<App />);
    const navigation = screen.getByRole("navigation", { name: "Prelude modes" });
    const version = within(navigation).getByLabelText("Prelude v2.6.3");
    expect(version.textContent).toBe("v2.6.3");
    expect(version.getAttribute("title")).toBe("Prelude v2.6.3");
  });
