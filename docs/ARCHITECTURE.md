...
Flashcards, Sequences, Ear Training, and Melody accept an optional read-only hosted Mobile Play state. When supplied, their local hooks remain dormant, their local entry/exit actions are absent, and their existing notation, keyboard, answer, and Melody review layouts render embedded rather than as competing fixed overlays. Without that input, standalone sessions retain their engine-owned Mobile Play lifecycle exactly as before; Free Play and Piece Practice are untouched. Exercise changes move focus to the new host context, and completed-run summaries receive focus without changing target achievement behavior. P6 snapshots, reducer events, stale-callback checks, progress, Bonus, dispositions, persistence, and one-engine mounting remain unchanged.
